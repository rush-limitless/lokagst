      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.expediteur === role ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[70%] rounded-lg px-3 py-2 text-sm ${m.expediteur === role ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-800"}`}>
              <p>{m.contenu}</p>
              <p className={`text-xs mt-1 ${m.expediteur === role ? "text-blue-200" : "text-gray-400"}`}>
